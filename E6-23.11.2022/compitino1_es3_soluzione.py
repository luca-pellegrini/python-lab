...
def main() :
    ...
    """
    print("DEBUG: funzione distance")
    print(distance("ACGTTGA", "AGCTTTA"))
    print(distance("ankn", "ajbn"))
    print(distance("abcd", "abcd"))
    print(distance("jbj", "d"))
    print(distance("abcd", "abef"))
    print(distance("abcd", "ebci"))
    print()
    
    print("DEBUG: funzione minDistance")
    print(minDistance("", ("a", "ab", "abd", "abc", "abc")))
    print(minDistance("", ( )))
    print(minDistance("abnk", ("bknk", "aooi")))
    print()
    print("DEBUG: funzione minGlobDist")
    s = minGlobDist(("ABSJ", "AJBO", "BBBA", "ABCD", "AFCD", "OPKN"))
    print(s)
    print(minGlobDist(("djkbk",)))
    print(minGlobDist(("jbkbk", "dsw")))
    """
# fine main
...
def minDistance(s, t) :
    pass 
    # SCRIVERE QUI   
    # Se s è la stringa vuota o t è una tupla vuota, restituisce la stringa vuota
    if len(s) == 0 or len(t) == 0 :
        return ''
    # Verifico che tutte le stringhe nella tupla t abbiano lunghezza = len(s)
    for x in t :
        if len(x) != len(s) :
            return ''
    
    M = len(t)
    shortestDistance = distance(s, t[0])
    stringShortestDistance = t[0]
    for i in range(1, M) :
        string = t[i]
        d = distance(s, string)
        if d < shortestDistance :
            shortestDistance = d
            stringShortestDistance = string
    return stringShortestDistance
#
# fine minDistance()
#
...
#  Le due stringhe da restituire devono costituire una tupla di 2 elementi.
#
...
def minGlobDist(t) :
    pass 
    # SCRIVERE QUI  
    # Se t e' una tupla vuota o una tupla di una sola stringa, restituisce una tupla vuota
    M = len(t)
    if M < 2 : return ( )
    for x in t :
        if len(x) != len(t[0]) :
    ...
    # Cerco la coppia di stringhe
    shortestDistance = 1 + len(t[0])
    for i in range(M) :
        s1 = t[i]
        tempList = list(t)
        tempList.remove(s1)
        s2 = minDistance(s1, tempList)
        d = distance(s1, s2)
        if d < shortestDistance :
            shortestDistance = d
            outputTuple = (s1, s2)
        print("DEBUG: shortestDistance =", shortestDistance)
        print("DEBUG: (a, b) =", outputTuple)
